...
class FriendGraph {
  ...
  areConnectedDFSRecursion(person1, person2, seen = new Set([person1])) {
    if (person1 === person2) return true
    for (let neighbor of person1.adjacent) {
      if (!seen.has(neighbor)) {
        seen.add(neighbor)
        if (this.areConnectedDFSRecursion(neighbor, person2, seen)) {
          return true
        }
      }
    }
...
friends.addPerson([moe, barney, lenny])
friends.setFriends(moe, barney)
friends.setFriends(barney, lenny)

console.log(friends.areConnectedDFSRecursion(homer, lisa))
